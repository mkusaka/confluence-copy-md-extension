...
export function normalizeSettings(settings: Partial<Record<keyof ExtensionSettings, unknown>>): ExtensionSettings {
  return {
    includeFrontmatter:
...
export async function loadSettings(storage: SettingsStorage = chrome.storage.sync): Promise<ExtensionSettings> {
